...
template <typename T>
void partition(T *input, int64_t length, T pivot, int k)
{
...
template <typename T>
void partition_int_by_key(int64_t *values, T *keys, int64_t length, T pivot, int k)
{
...
void top_k(T* d_vector, int length, int k, int blocks, int threads)
{
    ...
    partition(d_vector, length, pivot, k);
}
...
void top_k_int_by_key(int64_t* values, T* keys, int length, int k, int blocks, int threads)
{
    ...
    partition_int_by_key(values, keys, length, pivot, k);
}

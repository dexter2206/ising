...
def main(no_bits, num_states, log_level):
    ...
    py_result = search_py(Jh, 10)
    logger.info('Running ising CPU implementation')
    cpu = ising.search(Jh, num_states=num_states, method='CPU', omp_threads=12)
    sep = '+' + '+'.join(20 * '-' for _ in range(3)) + '+'
    print(sep)
    print('|{0:20}|{1:20}|'.format(' Python', ' ising (CPU)'))
    print(sep)
    for py_en, cpu_en in zip(next(iter(py_result)), sorted(cpu.energies)):
        print('|{0:20.5f}|{1:20.5f}|'.format(py_en, cpu_en))
